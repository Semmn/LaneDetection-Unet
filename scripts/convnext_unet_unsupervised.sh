...
train_config='/work/train/unsupervised/pixel_reconstruction/train_config/patch_masked_multidata.yaml'
...
project_name='convnext_unet_4x/culane/unsupervised'
weight_dir='/work/checkpoints'
plot_dir='/work/plots'
save_step=5
save_plot_step=5
load_weight_path='None' # only valid when start_epoch not 0
...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun --nproc_per_node=8 /work/train/unsupervised/pixel_reconstruction/multigpu_unsupervised.py \
    --batch_size $batch_size \
    --val_batch_size $val_batch_size \
    --model_config $model_config \
    --train_config $train_config \
    --mask_ratio $mask_ratio \
    --mask_window_h $mask_window_h \
    --mask_window_w $mask_window_w \
    --loss_type $loss_type \
    --no-multi_data_mode \
