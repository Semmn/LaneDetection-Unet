...
from model.head.unet import UNet as DefaultUNet
...
def get_model(opts):
    model_configs = load_yaml(opts.model_config)
    
    if opts.model == 'default_unet':
        ###############################################################################################
        ...
        ###############################################################################################
        in_channels = model_configs['DefaultUNet']['in_channels']
        out_channels = model_configs['DefaultUNet']['out_channels']
        
        encoder_channels = model_configs['DefaultUNet']['encoder']['channels']
        encoder_kernel_sz = model_configs['DefaultUNet']['encoder']['kernel_sz']
        encoder_padding = model_configs['DefaultUNet']['encoder']['padding']
        
        decoder_channels = model_configs['DefaultUNet']['decoder']['channels']
        decoder_kernel_sz = model_configs['DefaultUNet']['decoder']['kernel_sz']
        decoder_padding = model_configs['DefaultUNet']['decoder']['padding']
        
        latent_kernel_sz = model_configs['DefaultUNet']['latent']['kernel_sz']
        latent_padding = model_configs['DefaultUNet']['latent']['padding']
        
        model = DefaultUNet(in_channels=in_channels, encoder_channels=encoder_channels, encoder_kernel_sz=encoder_kernel_sz, 
                        encoder_padding=encoder_padding, decoder_channels=decoder_channels, decoder_kernel_sz=decoder_kernel_sz,
                        decoder_padding=decoder_padding, latent_kernel_sz=(latent_kernel_sz[0], latent_kernel_sz[1]),
                        latent_padding=(latent_padding[0], latent_padding[1]), out_channels=out_channels)
    elif opts.model == 'convnext_unet':
        ###################################################################################################
        ...
        model = UNet(encoder=unet_encoder, decoder=unet_decoder, num_cls=num_cls_config, output_mode=output_mode_config)
