...
# ConvNextV1REG
class ConvNextV1REG(torch.nn.Module):
    def __init__(self, backbone:ConvNextV1, device='cuda'):
        super().__init__()
        
        self.backbone=backbone
        self.device=device
        
        self.regressor_head = torch.nn.Linear(in_features=backbone.main_channels[-1], out_features=1, device=self.device)
        
    def forward(self, x):
        x = self.backbone(x)
        x = torch.nn.functional.avg_pool2d(x, x.size()[2:])
        x = x.flatten(start_dim=1, end_dim=3)
        return self.regressor_head(x) # output of regressor head is 1D vector (B, 1)
